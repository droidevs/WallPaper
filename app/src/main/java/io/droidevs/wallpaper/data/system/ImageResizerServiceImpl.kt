...
class ImageResizerServiceImpl(
    ...
    override suspend fun resize(bitmap: ImageData, method: ResizeMethod): ImageData {
        val (width, height) = getScreenDimensions()

        return processBitmap(bitmap){ internalBitmap,_ ->
            when (method) {
                ResizeMethod.CROP -> getResizedBitmap(internalBitmap, width, height)
                ResizeMethod.ZOOM -> getZoomedBitmap(internalBitmap, width, height)
                ResizeMethod.FIT_WIDTH -> getBitmapFitWidth(internalBitmap, width)
                ResizeMethod.FIT_HEIGHT -> getBitmapFitHeight(internalBitmap, height)
                ResizeMethod.NONE -> internalBitmap
            }
        }
    }
